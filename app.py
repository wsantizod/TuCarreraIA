...
El modelo se entrena en memoria al arrancar (no necesita escribir en disco).
...
def entrenar_modelo():
    """Entrena el modelo en memoria. No escribe nada en disco."""
    import pandas as pd
    ...
    rf.fit(X_sc, y_enc)
    print(f"Modelo entrenado en memoria. Carreras: {len(le.classes_)}")
    return rf, sc, le

print("Iniciando CareerAI — entrenando modelo...")
modelo, scaler, le = entrenar_modelo()
print("Listo.")

@app.route('/')
def index():
    return render_template('index.html', modelo_ok=True)
